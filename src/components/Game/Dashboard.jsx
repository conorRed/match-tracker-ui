...
export default function GameDashboard(props) {
  ...
  async function fetchAndSetGames(token) {
    ...
    let loadedGame = await apiResponse.json();
    setGame(loadedGame);
    updateScoreboards(
      loadedGame.events,
      loadedGame.home_team,
      loadedGame.away_team
    );
  }
  ...
  function updateScoreboards(events, home_team, away_team) {
    ...
    updatePoints(events, home_team, setHomePoints);
    updatePoints(events, away_team, setAwayPoints);
  }
